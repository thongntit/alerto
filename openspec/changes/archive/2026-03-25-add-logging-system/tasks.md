...
- [x] 1.1 Create `agent-alert/Services/AppLogger.swift` — define `LogLevel` (debug/info/warning/error) and `LogCategory` (http/notification/display/system) enums, both `CaseIterable` and `Comparable`
...
- [x] 2.1 Implement `persistToDisk()` in `AppLogger` — serialise `entries` as JSON to `~/Library/Application Support/AgentAlert/app.log.json` on a background `Task`; debounce writes within a 1-second window
...
- [x] 6.1 Create `agent-alert/Views/LogViewerView.swift` — `LogViewerView` as a SwiftUI `View` observing `AppLogger.shared`
...
- [x] 8.1 Add `AppLogger.swift` to the `agentic-notifier` target in `agent-alert.xcodeproj`
- [x] 8.2 Add `LogViewerView.swift` to the `agentic-notifier` target in `agent-alert.xcodeproj`
